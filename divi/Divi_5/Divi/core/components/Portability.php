...
class ET_Core_Portability {
	...
	public function serialize_layout( $id, $post_id, $content, $theme_builder_meta = array(), $chunk = 0 ) {
		...
		/**
		 * FIX for issue #44212: Escape literal newlines in ALL CSS fields.
		 *
		 * Theme Builder exports read content from database which contains literal newlines
		 * in CSS field values. These must be escaped for valid JSON export.
		 *
		 * Regular pages don't need this fix because frontend serialization (serializeModuleFlatObjects)
		 * already escapes newlines before sending to backend.
		 *
		 * Fields handled: freeForm, mainElement, before, after, fieldLabel, formField, formNotice, titleText
		 *
		 * @see https://github.com/elegantthemes/Divi/issues/44212
		 */
		foreach ( $data as $post_id => &$post_content ) {
			// Only process content that has CSS object AND literal newlines.
			if ( strpos( $post_content, '"css":{' ) !== false && strpos( $post_content, "\n" ) !== false ) {
				// Use regex to find and escape newlines in ALL CSS field values.
				// Pattern matches: "fieldName":"value" where fieldName is one of our CSS fields.
				// We escape newlines only within the value part, preserving JSON structure.
				$post_content = preg_replace_callback(
					'/("(?:freeForm|mainElement|before|after|fieldLabel|formField|formNotice|titleText)":")((?:[^"\\\\]|\\\\.)*?)(")/s',
					function( $matches ) {
						// Escape literal newlines and carriage returns in CSS value.
						$escaped_value = str_replace( "\n", "\\n", $matches[2] );
						$escaped_value = str_replace( "\r", "\\r", $escaped_value );
						// Reconstruct: opening + escaped value + closing.
						return $matches[1] . $escaped_value . $matches[3];
					},
					$post_content
				);
			}
