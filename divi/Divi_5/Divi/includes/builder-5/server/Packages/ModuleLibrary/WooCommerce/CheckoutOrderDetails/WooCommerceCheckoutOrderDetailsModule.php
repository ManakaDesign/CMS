...
use ET\Builder\Packages\StyleLibrary\Utils\MultiViewUtils;
...
class WooCommerceCheckoutOrderDetailsModule implements DependencyInterface {
	...
	public static function collapse_table_gutters_borders_style_declaration( array $params, array $attrs ): string {
		$collapse_table_gutters_borders = $params['attrValue'] ?? 'off';
		...
		if ( 'on' === $collapse_table_gutters_borders ) {
			...
			if ( ! empty( $breakpoint ) && ! empty( $state ) ) {
				$horizontal_gutter_width = $attrs['table']['advanced']['horizontalGutterWidth'][ $breakpoint ][ $state ] ?? '0px';
				$vertical_gutter_width   = $attrs['table']['advanced']['verticalGutterWidth'][ $breakpoint ][ $state ] ?? '0px';
			} else {
				...
				$horizontal_gutter_width = $attrs['table']['advanced']['horizontalGutterWidth'][ $default_breakpoint ][ $default_state ] ?? '0px';
				$vertical_gutter_width   = $attrs['table']['advanced']['verticalGutterWidth'][ $default_breakpoint ][ $default_state ] ?? '0px';
			}

			$style_declarations->add( 'border-collapse', 'separate' );
			$style_declarations->add( 'border-spacing', "{$horizontal_gutter_width} {$vertical_gutter_width}" );
		}
							...
							'styleProps' => [
								'advancedStyles' => [
									...
									[
										...
										'props'         => [
											'selector' => "{$order_class} table.shop_table",
											'attr'     => $attrs['table']['advanced']['collapseTableGuttersBorders'] ?? [],
