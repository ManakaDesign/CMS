...
class TransitionStyle {
	...
	public static function style( array $args ) {
		...
		// Check if transition attribute has explicit values.
		// This prevents auto-generating transitions for migrated D4 content that had hover/sticky states,
		// but no explicit transition settings. Maintains D4 parity where transitions were opt-in.
		// @see https://github.com/elegantthemes/Divi/issues/44498.
		$has_explicit_transition = ! empty( $attr );

		// JSON encode the attributes array for faster search using strpos and avoid any loops.
		// Only enable transitions if explicit transition attributes exist to avoid auto-generating
		// transitions for migrated D4 modules with hover states but no transition settings.
		$hover  = $has_explicit_transition && (bool) strpos( $attrs_json, 'hover' ); // Check if a module attribute has hover state.
		$sticky = $has_explicit_transition && (bool) strpos( $attrs_json, 'sticky' ); // Check if a module attribute has sticky state.
