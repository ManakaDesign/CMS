...
class BlockParserBlock extends \WP_Block_Parser_Block {
	...
	public function __construct( $name, $attrs, $inner_blocks, $inner_html, $inner_content, $store_instance, $parent_id = 'divi/root', $layout_type = 'default' ) {
		...
		if ( $this->blockName ) {
			...
			// Skip incrementing order index during global layout template parsing.
			// When parsing templates to extract content, we don't want to consume indices.
			if ( ! empty( $_is_parsing_global_layout ) ) {
				// Use a negative temporary order index that won't interfere with actual indexing.
				$this->orderIndex = -1;
			} else {
