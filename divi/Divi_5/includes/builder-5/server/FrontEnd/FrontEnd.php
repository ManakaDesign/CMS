...
class FrontEnd {
	 ...
	 * et_pb_code_inner divs. This is the D5 equivalent of D4's fix_wptexturized_scripts.
	 *
	 * @since ??
	 *
	...
	public function fix_code_module_wptexturize( string $content ): string {
		// Only process if contains Code Module content.
		if ( false === strpos( $content, 'et_pb_code_inner' ) ) {
			return $content;
		}

		// Find and fix Code Module content that was wptexturized.
		return preg_replace_callback(
			'/<div[^>]*class="[^"]*et_pb_code_inner[^"]*"[^>]*>(.*?)<\/div>/s',
			function( $matches ) {
