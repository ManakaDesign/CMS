...
class BlockParser extends \WP_Block_Parser {
	...
	public function parse( $document ) {
		...
		$reset_order_index = ! $is_rendering_inner_content;
		 ...
		 * during block parsing. By default, order indexes are reset when parsing main document
		 * content. When parsing inner content (such as rendering Blog module content), existing
		 * order indexes are preserved to prevent conflicts between main content and inner content
		 * CSS class numbering.
		 *
		 * @since ??
		 *
		 * @param bool $reset_order_index Whether to reset order indexes. Default is true when
		 *                                rendering main content, otherwise false.
		 */
		$reset_order_index = apply_filters( 'divi_front_end_block_parser_reset_order_index', $reset_order_index );
