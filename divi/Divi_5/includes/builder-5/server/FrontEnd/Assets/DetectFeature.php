...
class DetectFeature {
	 ...
	 * setting in Gutenberg block content.
	 *
	 * @since ??
	 *
	...
	public static function has_css_grid_layout_enabled( string $content, array $options = [] ): bool {
		 ...
		 * test regex: https://regex101.com/r/pNeRvU/1
		 */
		static $pattern = '"layout":{.*?"display":"grid".*?}';

		// Perform a single regex search for CSS Grid layout.
		preg_match( "~$pattern~", $content, $matches );

		// Return true if any CSS Grid layout was detected, false otherwise.
		return ! empty( $matches );
	}
	 ...
	 * ensuring reCaptcha scripts only need to be loaded when spam protection is actually enabled.
	 *
	 * @since ??
	 *
	...
	public static function has_recaptcha_enabled( string $content, array $options = [] ): bool {
		...
		// Quick check for common patterns before doing expensive regex operations.
		if ( false === strpos( $content, 'spamProtection' ) && false === strpos( $content, 'use_spam_service' ) ) {
			return false;
		}

		// Check for D5 JSON format: `"spamProtection":{"desktop":{"value":{"enabled":"on"}}}`.
		// Pattern matches: "spamProtection" followed by "desktop" -> "value" -> "enabled":"on".
		$d5_pattern = '/"spamProtection"\s*:\s*\{\s*"desktop"\s*:\s*\{\s*"value"\s*:\s*\{\s*"enabled"\s*:\s*"on"/i';
		if ( preg_match( $d5_pattern, $content ) ) {
			return true;
		}
