...
class ModuleUtils {
	...
	public static function get_and_inherit_background_attr( array $args ): array {
		...
		if ( ! empty( $attr_to_be_returned ) ) {
			...
			$breakpoint_names          = \ET\Builder\Framework\Breakpoint\Breakpoint::get_enabled_breakpoint_names();
	 ...
	 */
	public static function is_parent_grid_layout( string $module_id ): bool {
		$parent = BlockParserStore::get_parent( $module_id );
			...
			if ( 'post' === $post_type ) {
				...
				foreach ( $categories_normalized as $category ) {
					if ( 'current' === $category ) {
						...
						$current_post_id = self::_get_current_post_id_for_category_filtering();
						if ( $current_post_id > 0 ) {
							$post_terms = wp_get_object_terms( $current_post_id, 'category' );
				...
				foreach ( $categories_normalized as $category ) {
					if ( 'current' === $category ) {
						// Get taxonomy terms from the current post using D4 pattern with Theme Builder support.
						$current_post_id = self::_get_current_post_id_for_category_filtering();
						if ( $current_post_id > 0 ) {
							$post_terms = wp_get_object_terms( $current_post_id, $taxonomy );
							if ( is_wp_error( $post_terms ) ) {
								continue;
							}
							$current_term_ids    = wp_list_pluck( $post_terms, 'term_id' );
